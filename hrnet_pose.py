...
class PoseDetector:
    def __init__(self, gpu_id=0):
        self.CTX = torch.device('cuda:%d' % gpu_id) if torch.cuda.is_available() else torch.device('cpu')
    ...
    def detect(self, image_bgr, draw=False):
        ...
        # object detection box
        pred_boxes = get_person_detection_boxes(self.box_model, input, threshold=0.9)
        pred_poses = []

...
if __name__ == "__main__":
    ...
    root = "/home/summer/software/aug2/"                               # Root path to one recording session
    ...
    #             os.rename(os.path.join(path, file), os.path.join(path, ''.join([file[:1], file[file.rfind('_'):]])))
    #         elif file.startswith("1_"):
    #             os.rename(os.path.join(path, file), os.path.join(path, ''.join([file[:1], file[file.rfind('_'):]])))
    for i in range(1, 5):
        for cam in ["0", "1"]:
            path = root + "session_" + str(session_id) + "/sensor_" + str(i) + '/video/' + cam + "__%d.avi"
